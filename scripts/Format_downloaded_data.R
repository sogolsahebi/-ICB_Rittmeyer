...
# - Creates "CLIN.txt" ,dimension 699 x 13
# - Creates "EXPR.txt.gz", dimension 28824 x 699

# Load required libraries
library(data.table)

# Read clinical data
clin1 <- read.csv("source data/EGAD00001008549/EGAF00005797822/go28915_anon_subsetted_BYN_n699.csv") # dim 699 x 10
clin2 <- read.csv("source data/EGAD00001008549/EGAF00006143195/ss.csv") # 891 x 11
...
clin_merged <- merge(clin1, clin2, by.x = "UNI_ID", by.y = "subjectId") # dim 699 x 13
...
expr <- as.data.frame(fread("source data/EGAD00001008391/EGAF00005797824/anonymized_OAK-TPMs2.csv")) 
...
expr <- expr[sort(rownames(expr)), ] # dim 28824 x 699
